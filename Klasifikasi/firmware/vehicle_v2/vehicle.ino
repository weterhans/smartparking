/**
 * ============================================================
 *  EDGE AI VEHICLE CLASSIFIER — v2.0
 *  Perbaikan utama:
 *   1. Sampling BERHENTI saat sensor 2 terpicu
 *   2. Buffer siluet bertimestamp → rekonstruksi spasial akurat
 *   3. Frame parser TF-Luna yang benar (header + checksum)
 *   4. Kalibrasi yang benar dengan validasi frame penuh
 *   5. Perhitungan kecepatan lebih robust (guard v_keluar)
 *   6. Reset yang aman (preservasi offset kalibrasi)
 * ============================================================
 */
...
const float  ALPHA_FILTER          = 0.50f;    // EMA filter

// Kompensasi lag EMA di kedua ujung deteksi (masuk & keluar)
// Pada α=0.5 dan 100Hz, lag ≈ 1.4 sample per transisi ≈ 14 ms per ujung
const float  EMA_LAG_KOMPENSASI_US = 14000.0f; // µs per ujung
...
int  klasifikasiKNN_K(float dataBaru[8], int K);
void evaluasiFitur();
void resetSistem();
void cekTimeout();
void yieldDelay(unsigned long ms);
...
void setup() {
  ...
  lcd.setCursor(0, 1); lcd.print("TF-Luna v2.0");
  ...
  while (lidarSerial.available() > 0) {
    ...
    if (s.dataIndex == 9) {
      ...
      if (!bacaanInvalid) {
        ...
        s.jarakMulus       = ALPHA_FILTER * rawDist + (1.0f - ALPHA_FILTER) * s.jarakMulus;
        ...
        // Bacaan invalid (kaca): pertahankan LKG, jangan update filter EMA
        s.jumlahKaca++;
        if (!s.bacaanKacaAktif) {
        ...
        // jarakMulus tidak diupdate, biarkan nilai terakhir yang valid
      }
...
void jalankanKalibrasiAwal(HardwareSerial &lidar, LidarData &s, float targetJarak) {
  ...
  s.jarakMulus      = targetJarak; // inisialisasi filter EMA
}
...
void evaluasiFitur() {
  ...
  // ── ANALISIS MULTI-K (Untuk Keperluan Penelitian) ──
  // Jalankan KNN dengan berbagai nilai K sekaligus untuk perbandingan akurasi
  const int nilaiK_uji[] = {1, 3, 5, 7};
  int hasilMultiK[4];
  for (int ki = 0; ki < 4; ki++) {
    hasilMultiK[ki] = klasifikasiKNN_K(fiturBaru, nilaiK_uji[ki]);
  }
  ...
  out += "───────────────────────────────────────────\n";
  out += "[MULTI-K ANALISIS]\n";
  for (int ki = 0; ki < 4; ki++) {
    String label = (hasilMultiK[ki] >= 0) ? NAMA_KELAS[hasilMultiK[ki]] : "UNKNOWN";
    out += "  K=" + String(nilaiK_uji[ki]) + " → " + label;
    if (nilaiK_uji[ki] == NILAI_K) out += "  ← (aktif)";
    out += "\n";
  }
  out += "═══════════════════════════════════════════\n";
  wlog(out);
...
// ─────────────────────────────────────────────
//  KNN DENGAN NILAI K VARIABEL (Untuk Analisis Multi-K)
// ─────────────────────────────────────────────
int klasifikasiKNN_K(float dataBaru[8], int K) {
  if (K < 1) K = 1;
  if (K > JUMLAH_DATA) K = JUMLAH_DATA;

  // 1. Min-Max normalisasi data baru
  float dataNorm[8];
  ...
  // 2. Alokasi array dinamis untuk K terdekat
  // Gunakan buffer statis dengan ukuran maksimal (K maks = 7)
  const int K_MAKS = 7;
  float jarakTerdekat[K_MAKS];
  int   labelTerdekat[K_MAKS];
  for (int i = 0; i < K; i++) {
    jarakTerdekat[i] = 1e30f;
    labelTerdekat[i] = -1;
  }

  // 3. Hitung jarak ke seluruh dataset
...
void resetSistem() {
  ...
  float mulus1  = s1.jarakMulus;    // pertahankan filter EMA
  ...
  s1.offsetKalibrasi = offset1;  s1.jarakMulus = mulus1;
  s2.offsetKalibrasi = offset2;  s2.jarakMulus = mulus2;
