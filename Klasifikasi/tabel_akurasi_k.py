...
"""
=============================================================
  TABEL AKURASI KNN — CONFUSION MATRIX PER K
  Menghitung KNN secara nyata & menghasilkan Tabel Word
  Format: seperti contoh skripsi (Nilai K | CM per kelas | Akurasi)
=============================================================
"""
import math
...
# ─────────────────────────────────────────────────────────────────
# 1. DATA LATIH (dari vehicle.ino — 75 data, 8 fitur, 5 kelas)
# ─────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────
# 2. DATA UJI (dari tabel pengujian nyata Anda)
# ─────────────────────────────────────────────────────────────────
# (fitur, label_benar, nama_kendaraan)
test_data = [
    # MPV
...
# ─────────────────────────────────────────────────────────────────
# 3. MIN-MAX & BOBOT (dari vehicle.ino)
# ─────────────────────────────────────────────────────────────────
...
NAMA_KELAS = ["CityCar", "Sedan", "MPV", "SUV", "Pickup"]
N_KELAS = 5

def normalize(f):
    return [(f[i] - F_MIN[i]) / (F_MAX[i] - F_MIN[i]) for i in range(8)]

def weighted_euclidean(a, b):
    return math.sqrt(sum(BOBOT[i] * (a[i] - b[i])**2 for i in range(8)))

def predict_knn(fitur_uji, k):
    norm_uji = normalize(fitur_uji)
    dists = []
    for (fitur_latih, label) in train_data:
        norm_latih = normalize(fitur_latih)
        d = weighted_euclidean(norm_uji, norm_latih)
...
# ─────────────────────────────────────────────────────────────────
# 4. HITUNG CONFUSION MATRIX PER K
# ─────────────────────────────────────────────────────────────────
def hitung_cm(k):
    total = len(test_data)
    benar = 0
    # TP, FP, FN, TN per kelas
    cm = {c: {"TP": 0, "FP": 0, "FN": 0, "TN": 0} for c in range(N_KELAS)}
    
    for (fitur, label_benar, nama) in test_data:
        pred = predict_knn(fitur, k)
        if pred == label_benar:
            benar += 1
        for c in range(N_KELAS):
            if label_benar == c and pred == c:
                cm[c]["TP"] += 1
            elif label_benar != c and pred == c:
                cm[c]["FP"] += 1
            elif label_benar == c and pred != c:
                cm[c]["FN"] += 1
            else:
    ...
    return cm, benar, total, akurasi

# Hitung untuk K = 1, 3, 5, 7
K_VALUES = [1, 3, 5, 7]
hasil = {}
for k in K_VALUES:
    cm, benar, total, acc = hitung_cm(k)
    hasil[k] = {"cm": cm, "benar": benar, "total": total, "akurasi": acc}
    print(f"\nK = {k} -> {benar}/{total} = {acc}%")
    for c in range(N_KELAS):
        m = cm[c]
        print(f"  {NAMA_KELAS[c]:8}: TP={m['TP']}, TN={m['TN']}, FP={m['FP']}, FN={m['FN']}")
...
run = p.add_run("Tabel 11. Akurasi Berbagai Nilai K")
run.bold = True
...
for cell, text in zip(hdr, ["Nilai K", "Confusion Matrix", "Akurasi"]):
    cell.text = text
...
for k in K_VALUES:
    ...
    # Kolom 2: Confusion Matrix per kelas (monospaced-like)
    p2 = row[1].paragraphs[0]
    p2.alignment = WD_ALIGN_PARAGRAPH.LEFT
    for c in range(N_KELAS):
        m = h["cm"][c]
        line = f"{NAMA_KELAS[c]}: TP={m['TP']}, TN={m['TN']}, FP={m['FP']}, FN={m['FN']}"
        if c == 0:
...
# Simpan
out = "Tabel_Akurasi_K.docx"
doc.save(out)
print(f"\nOK - Tabel disimpan: {out}")
